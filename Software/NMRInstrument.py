    ...
    ###################################################
    ### NMRInstrument - LSDRM - Corentin COUTELLIER ###
    ###################################################
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def update_plot_data(self):
        ...
        self.time.append(datetime.timestamp(datetime.now()))
...
# class SecondKitWindow(MainWindow):

#     def __init__(self, *args, **kwargs):
#         super(SecondKitWindow, self).__init__(*args, **kwargs)

#         self.setWindowTitle("NMRinstrument - Second Kit")
#         global deviceMem
...
#         self.thermocoupleType = QtWidgets.QComboBox()
...
#         if self.adsDevice.liveRegMemory[4] & (1 << i):
...
#             1: 0xD0
#         }
#         self.adsDevice.setReg(reg, reset=False)
#         binVal = self.adsDevice.command(ads.START_RESTART)
#         total_mV = self.adsDevice.binToVolt(binVal, ads.ADSinternalREF * 1000) / (pow(2, (reg.get(0) & 0x0E) >> 1))
#         coldJunctionTemp = self.adsDevice.getTemp(reg.get(1)) - 1       # We substract 1, because there is approximatively 1 Kelvin difference between the real cold junction, and the ADS122U04 integrated temperature sensor measurement.
...
#         self.adsDevice.setReg(self.regValues, list(reg.keys()), False)      # Set back regValues that were modified.
...
class DataPlot(QtWidgets.QMainWindow):
    ...
    def __init__(self, ID, *args, **kwargs):        # , xName, x, yName, y, 
        ...
        self.setWindowTitle("NMRInstrument - Data plot window - " + self.fileName[0])
    ...
    def LoadData(self):
        ...
        if settings.firstExec:
            settings.url = QtCore.QUrl("clsid:0AC0837C-BBF8-452A-850D-79D08E667CA7")     # This refer to "Ce PC" shortcut, to more CLSID shortcut check https://learn.microsoft.com/en-us/windows/win32/shell/knownfolderid
            dialog.setDirectoryUrl(settings.url)
        else:
            dialog.setDirectory(settings.url)

        if not self.refreshMode:
            self.fileName = dialog.getOpenFileName(filter="Kit File (*.kit)")
        
        if self.fileName[0] != '':
            settings.url = self.fileName[0]
            settings.firstExec = False
            file = open(settings.url, 'r')
...
class SetupWindow(QtWidgets.QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.setWindowTitle("NMRinstrument - choose your kit")
        ...
        for x in os.listdir(os.path.dirname(__file__) + '/Kits'):
            if x.endswith(".py"):
                x = x[:-3]
        ...
        self.debugGUIbutton = QtWidgets.QPushButton("Debug GUI")
        self.debugGUIbutton.clicked.connect(self.showDebugGUI)
        ...
        for x in self.kitsList:
            self.mainLayout.addWidget(self.selectButton[self.kitsList.index(x)], row, column)
            row += column
            column = not column
        row += 1
        column = 0

        self.mainLayout.addWidget(self.debugGUIbutton, row, column, 1, 2)
        self.mainLayout.addWidget(self.personalizButton, row+1, column, 1, 2)
        self.mainLayout.addWidget(self.loadDataButton, row+2, column, 1, 2)
    ...
    def showDebugGUI(self):
        self.DebugWin = DebugWindow()
        self.DebugWin.showMaximized()
    ...
    def showKit(self, name):
        ...
        kit = importlib.import_module("Kits." + name)
        try:
            if kit.deviceConnection():
                self.KitWin = kit.KitWindow()
    ...
    def showDataPlot(self):
        settings.DataWindow.append(DataPlot(settings.dataPlotID))
        if settings.DataWindow[-1].showData:
            settings.dataPlotID += 1
            settings.DataWindow[-1].show()
        else:
            settings.DataWindow = settings.DataWindow[:-1]


    # def deviceConnection(self):
    #     self.devList = ftd.listDevices()
    #     if not self.devList:
    #         Dialog(0x04)
    #         return False
    #     else:
...
class Dialog(QtWidgets.QMessageBox):
    ...
    def __init__(self, errorCode, *args, **kwargs):
        ...
        if self.errorCode == 0x00:
            ...
            self.setWindowTitle("No FTDI device found")
            self.setText("No FTDI device detected. Please, connect your device before selecting a kit.")

        elif self.errorCode == 0x05:
            self.setWindowTitle("Too much points to smooth")
            self.setText("The number of points to smooth cannot exceed the total number of points.")
        ...
        elif self.errorCode == 0x08:
            self.setWindowTitle("Too much points to interlace")
            self.setText("The number of points to interlace cannot exceed or be equal to the total number of points to smooth.")

        elif self.errorCode == 0x09:
            self.setWindowTitle("Unable to communicate with the kit board")
            self.setText("The ADC registers can't be set or too many timeout occurs.\nPlease, make sure the instrumentation board is powered and connected to the USB-UART module.")
            self.setStandardButtons(QtWidgets.QMessageBox.Retry | QtWidgets.QMessageBox.Cancel)
            self.setIcon(QtWidgets.QMessageBox.Warning)
            self.needAnswer = True

        elif self.errorCode == 0x0A:
        ...
        elif self.errorCode == 0x0B:
            self.setWindowTitle("Thermocouple disconnected")
            self.setText("The thermocouple seems to be disconnected. Connect it back to resume data acquisition.")
            self.setStandardButtons(QtWidgets.QMessageBox.Retry | QtWidgets.QMessageBox.Cancel)
            self.setIcon(QtWidgets.QMessageBox.Warning)
            self.needAnswer = True

        elif self.errorCode == 0x0C:
            self.setWindowTitle("Unrecognized module")
            self.setText("A class or method seems to miss to the selected module (.py file in " + os.path.dirname(os.path.realpath(__file__)) + "\\Kits\\).")
