...
class Device:
    ...
    def getTemp(self, regVal=None):    # Return the internal temperature measured by the integrated precision temperature sensor, in Kelvin.
        if regVal is None:
            regVal = self.command(RREG[1])

        if not (regVal & 0x01):
            self.command(WREG[1], regVal | 0x01)

        binValue = self.command(START_RESTART) & 0xFFFFFF   # The "& 0xFFFFFF" is to ensure that binValue is on 24 bits.
    ...
    def setReg(self, regValues, regToWrite=None, reset=True):    #Write registers values
