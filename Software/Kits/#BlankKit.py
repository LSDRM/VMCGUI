...
# Every classes, methods and variables already writen in this file are essentials to VMCS, if VMCS.py has not been modified.
...
class KitWindow(MainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.setWindowTitle("VMCS - Put the title of your window here")

        self.prefix = "Here the default prefix for your files"
        self.closeStartWindowOnLaunch = True    # With this version of VMCS, I recommend to keep this to True, to prevent multiple windows trying to access to the the same port/communication interface.
        ...
        self.initCentralFrame()     # Call the initCentralFrame() method from the MainWindow class in VMCS.py will automatically generate the layout according to the number of sensors and GPIOs.
        
        self.timer = QtCore.QTimer()
        self.timer.setInterval(self.sampleTime.value() * self.sampleUnitMultiplier)
        self.timer.timeout.connect(self.update_plot_data)   # Call udate_plot_data() method from VMCS.py each times the interval time is reached.
        self.timer.start()
    ...
    def checkGPIOstate(self, i):    # If you don't have any GPIO in your system, and set self.GPIOsNumber to 0, you can safely remove this method.
        if GPIO[i].isActive():      # HERE, you must replace GPIO[i].isActive() with the instruction you'll use to check if the GPIO with identifier 'i' is active.
    ...
    def manualSetGPIO(self, i):     # If you don't have any GPIO in your system, and set self.GPIOsNumber to 0, you can safely remove this method.
        ...
        self.applyGPIO(i)
        # Here, you may replace the whole line to set the state of GPIO with identifier 'i' to the value of "self.outputManualSetButton[i].isChecked()" which is boolean.
    ...
    def writeMeasurements(self, pos):       # Write a line of measurements in the opened file
        ...
        # The line above is used when recording measurements, it write the measure of each sensor in the selected file.



def deviceConnection():     # This method serve to check if 
